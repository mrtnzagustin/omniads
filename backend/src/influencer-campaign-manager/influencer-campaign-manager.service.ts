...
export class InfluencerCampaignManagerService {
  constructor(
    @InjectRepository(InfluencerCampaignManager)
    private readonly repository: Repository<InfluencerCampaignManager>,
  ) {}

  async create(userId: string, data: Partial<InfluencerCampaignManager>): Promise<InfluencerCampaignManager> {
  ...
  async findAllByUser(userId: string): Promise<InfluencerCampaignManager[]> {
  ...
  async findOne(id: string): Promise<InfluencerCampaignManager> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`InfluencerCampaignManager with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<InfluencerCampaignManager>): Promise<InfluencerCampaignManager> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'InfluencerCampaignManager Analysis',
