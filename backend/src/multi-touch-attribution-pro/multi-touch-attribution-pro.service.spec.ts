...
describe('MultiTouchAttributionProService', () => {
  let service: MultiTouchAttributionProService;
  let repository: Repository<CustomerJourney106>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MultiTouchAttributionProService,
        {
          provide: getRepositoryToken(CustomerJourney106),
          useValue: mockRepository,
        },
      ],
    }).compile();

    service = module.get<MultiTouchAttributionProService>(MultiTouchAttributionProService);
    repository = module.get<Repository<CustomerJourney106>>(getRepositoryToken(CustomerJourney106));
  });
