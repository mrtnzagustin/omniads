...
describe('DynamicVideoRemixerService', () => {
  let service: DynamicVideoRemixerService;
  let repository: Repository<VideoAsset089>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DynamicVideoRemixerService,
        {
          provide: getRepositoryToken(VideoAsset089),
          useValue: mockRepository,
        },
      ],
    }).compile();

    service = module.get<DynamicVideoRemixerService>(DynamicVideoRemixerService);
    repository = module.get<Repository<VideoAsset089>>(getRepositoryToken(VideoAsset089));
  });
