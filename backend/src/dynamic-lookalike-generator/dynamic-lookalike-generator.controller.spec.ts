...
describe('DynamicLookalikeGeneratorController', () => {
  let controller: DynamicLookalikeGeneratorController;
  let service: jest.Mocked<DynamicLookalikeGeneratorService>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        {
          provide: DynamicLookalikeGeneratorService,
          useValue: mockService,
        },
      ],
    }).compile();

    controller = module.get<DynamicLookalikeGeneratorController>(DynamicLookalikeGeneratorController);
    service = module.get(DynamicLookalikeGeneratorService);
  });
  ...
  describe('delete', () => {
    it('should call service and return success message', async () => {
      ...
      expect(result).toEqual({ message: 'Dynamic lookalike generator deleted successfully' });
    });
  });
});
