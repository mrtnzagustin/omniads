...
describe('CrossDeviceAttributionService', () => {
  let service: CrossDeviceAttributionService;
  let repository: Repository<CrossDeviceAttribution>;
  ...
  const mockEntity: Partial<CrossDeviceAttribution> = {
    id: '123',
    userId: 'user1',
    name: 'Test Entity',
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        CrossDeviceAttributionService,
        {
          provide: getRepositoryToken(CrossDeviceAttribution),
          useValue: mockRepository,
        },
      ],
    }).compile();

    service = module.get<CrossDeviceAttributionService>(CrossDeviceAttributionService);
    repository = module.get<Repository<CrossDeviceAttribution>>(getRepositoryToken(CrossDeviceAttribution));
  });
