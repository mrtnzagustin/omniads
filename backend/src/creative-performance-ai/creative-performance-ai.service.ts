...
export class CreativePerformanceAiService {
  constructor(
    @InjectRepository(CreativePerformanceAi)
    private readonly repository: Repository<CreativePerformanceAi>,
  ) {}

  async create(userId: string, data: Partial<CreativePerformanceAi>): Promise<CreativePerformanceAi> {
  ...
  async findAllByUser(userId: string): Promise<CreativePerformanceAi[]> {
  ...
  async findOne(id: string): Promise<CreativePerformanceAi> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`CreativePerformanceAi with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<CreativePerformanceAi>): Promise<CreativePerformanceAi> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'CreativePerformanceAi Analysis',
