...
describe('AiTestDesignerController', () => {
  let controller: AiTestDesignerController;
  let service: jest.Mocked<AiTestDesignerService>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        {
          provide: AiTestDesignerService,
          useValue: mockService,
        },
      ],
    }).compile();

    controller = module.get<AiTestDesignerController>(AiTestDesignerController);
    service = module.get(AiTestDesignerService);
  });
  ...
  describe('delete', () => {
    it('should call service and return success message', async () => {
      ...
      expect(result).toEqual({ message: 'AI test designer deleted successfully' });
    });
  });
});
