...
export class EmailAdSyncEngineService {
  constructor(
    @InjectRepository(EmailAdSyncEngine)
    private readonly repository: Repository<EmailAdSyncEngine>,
  ) {}

  async create(userId: string, data: Partial<EmailAdSyncEngine>): Promise<EmailAdSyncEngine> {
  ...
  async findAllByUser(userId: string): Promise<EmailAdSyncEngine[]> {
  ...
  async findOne(id: string): Promise<EmailAdSyncEngine> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`EmailAdSyncEngine with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<EmailAdSyncEngine>): Promise<EmailAdSyncEngine> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'EmailAdSyncEngine Analysis',
