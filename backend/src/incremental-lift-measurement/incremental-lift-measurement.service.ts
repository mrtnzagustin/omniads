...
export class IncrementalLiftMeasurementService {
  constructor(
    @InjectRepository(IncrementalLiftMeasurement)
    private readonly repository: Repository<IncrementalLiftMeasurement>,
  ) {}

  async create(userId: string, data: Partial<IncrementalLiftMeasurement>): Promise<IncrementalLiftMeasurement> {
  ...
  async findAllByUser(userId: string): Promise<IncrementalLiftMeasurement[]> {
  ...
  async findOne(id: string): Promise<IncrementalLiftMeasurement> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`IncrementalLiftMeasurement with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<IncrementalLiftMeasurement>): Promise<IncrementalLiftMeasurement> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'IncrementalLiftMeasurement Analysis',
