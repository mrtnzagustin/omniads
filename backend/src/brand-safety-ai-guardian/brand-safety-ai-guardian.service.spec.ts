...
describe('BrandSafetyAiGuardianService', () => {
  let service: BrandSafetyAiGuardianService;
  let repository: Repository<BrandSafetyRule105>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BrandSafetyAiGuardianService,
        {
          provide: getRepositoryToken(BrandSafetyRule105),
          useValue: mockRepository,
        },
      ],
    }).compile();

    service = module.get<BrandSafetyAiGuardianService>(BrandSafetyAiGuardianService);
    repository = module.get<Repository<BrandSafetyRule105>>(getRepositoryToken(BrandSafetyRule105));
  });
