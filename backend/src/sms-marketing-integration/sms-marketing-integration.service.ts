...
export class SmsMarketingIntegrationService {
  constructor(
    @InjectRepository(SmsMarketingIntegration)
    private readonly repository: Repository<SmsMarketingIntegration>,
  ) {}

  async create(userId: string, data: Partial<SmsMarketingIntegration>): Promise<SmsMarketingIntegration> {
  ...
  async findAllByUser(userId: string): Promise<SmsMarketingIntegration[]> {
  ...
  async findOne(id: string): Promise<SmsMarketingIntegration> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`SmsMarketingIntegration with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<SmsMarketingIntegration>): Promise<SmsMarketingIntegration> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'SmsMarketingIntegration Analysis',
