...
describe('ServerSideTrackingService', () => {
  let service: ServerSideTrackingService;
  let repository: Repository<ServerSideTracking>;
  ...
  const mockEntity: Partial<ServerSideTracking> = {
    id: '123',
    userId: 'user1',
    name: 'Test Entity',
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ServerSideTrackingService,
        {
          provide: getRepositoryToken(ServerSideTracking),
          useValue: mockRepository,
        },
      ],
    }).compile();

    service = module.get<ServerSideTrackingService>(ServerSideTrackingService);
    repository = module.get<Repository<ServerSideTracking>>(getRepositoryToken(ServerSideTracking));
  });
