...
export class CustomerJourneyMapperService {
  constructor(
    @InjectRepository(CustomerJourneyMapper)
    private readonly repository: Repository<CustomerJourneyMapper>,
  ) {}

  async create(userId: string, data: Partial<CustomerJourneyMapper>): Promise<CustomerJourneyMapper> {
  ...
  async findAllByUser(userId: string): Promise<CustomerJourneyMapper[]> {
  ...
  async findOne(id: string): Promise<CustomerJourneyMapper> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`CustomerJourneyMapper with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<CustomerJourneyMapper>): Promise<CustomerJourneyMapper> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'CustomerJourneyMapper Analysis',
