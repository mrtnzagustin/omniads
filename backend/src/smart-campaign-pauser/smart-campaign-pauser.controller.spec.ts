...
describe('SmartCampaignPauserController', () => {
  let controller: SmartCampaignPauserController;
  let service: jest.Mocked<SmartCampaignPauserService>;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        {
          provide: SmartCampaignPauserService,
          useValue: mockService,
        },
      ],
    }).compile();

    controller = module.get<SmartCampaignPauserController>(SmartCampaignPauserController);
    service = module.get(SmartCampaignPauserService);
  });
  ...
  describe('delete', () => {
    it('should call service and return success message', async () => {
      ...
      expect(result).toEqual({ message: 'Smart campaign pauser deleted successfully' });
    });
  });
});
