...
export class AudienceOverlapAnalyzerService {
  constructor(
    @InjectRepository(AudienceOverlapAnalyzer)
    private readonly repository: Repository<AudienceOverlapAnalyzer>,
  ) {}

  async create(userId: string, data: Partial<AudienceOverlapAnalyzer>): Promise<AudienceOverlapAnalyzer> {
  ...
  async findAllByUser(userId: string): Promise<AudienceOverlapAnalyzer[]> {
  ...
  async findOne(id: string): Promise<AudienceOverlapAnalyzer> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`AudienceOverlapAnalyzer with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<AudienceOverlapAnalyzer>): Promise<AudienceOverlapAnalyzer> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'AudienceOverlapAnalyzer Analysis',
