...
export class ProfitAnalyticsDashboardService {
  constructor(
    @InjectRepository(ProfitAnalyticsDashboard)
    private readonly repository: Repository<ProfitAnalyticsDashboard>,
  ) {}

  async create(userId: string, data: Partial<ProfitAnalyticsDashboard>): Promise<ProfitAnalyticsDashboard> {
  ...
  async findAllByUser(userId: string): Promise<ProfitAnalyticsDashboard[]> {
  ...
  async findOne(id: string): Promise<ProfitAnalyticsDashboard> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`ProfitAnalyticsDashboard with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<ProfitAnalyticsDashboard>): Promise<ProfitAnalyticsDashboard> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'ProfitAnalyticsDashboard Analysis',
