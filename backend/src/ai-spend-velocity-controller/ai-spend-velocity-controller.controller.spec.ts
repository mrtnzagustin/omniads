...
describe('AiSpendVelocityControllerController', () => {
  let controller: AiSpendVelocityControllerController;
  let service: jest.Mocked<AiSpendVelocityControllerService>;
  ...
  const mockEntity = {
    ...
    configuration: {
      ...
      timeframe: 'daily',
      velocityThresholds: { slowdown: 800, optimal: 1000, accelerate: 1200 },
      budgetConstraints: { minDailySpend: 500, maxDailySpend: 2000, totalBudget: 30000 },
      performanceGates: [],
      adjustmentRules: []
    },
    metrics: null,
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        {
          provide: AiSpendVelocityControllerService,
          useValue: mockService,
        },
      ],
    }).compile();

    controller = module.get<AiSpendVelocityControllerController>(AiSpendVelocityControllerController);
    service = module.get(AiSpendVelocityControllerService);
  });
  ...
  describe('analyze', () => {
    it('should call service and return analysis', async () => {
      const analysis = { id: '123', velocityAnalysis: {}, performance: {}, recommendations: [] };
  ...
  describe('delete', () => {
    it('should call service and return success message', async () => {
      ...
      expect(result).toEqual({ message: 'AI spend velocity controller deleted successfully' });
    });
  });
});
