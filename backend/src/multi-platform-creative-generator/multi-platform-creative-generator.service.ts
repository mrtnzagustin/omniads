...
export class MultiPlatformCreativeGeneratorService {
  constructor(
    @InjectRepository(MultiPlatformCreativeGenerator)
    private readonly repository: Repository<MultiPlatformCreativeGenerator>,
  ) {}

  async create(userId: string, data: Partial<MultiPlatformCreativeGenerator>): Promise<MultiPlatformCreativeGenerator> {
  ...
  async findAllByUser(userId: string): Promise<MultiPlatformCreativeGenerator[]> {
  ...
  async findOne(id: string): Promise<MultiPlatformCreativeGenerator> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`MultiPlatformCreativeGenerator with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<MultiPlatformCreativeGenerator>): Promise<MultiPlatformCreativeGenerator> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'MultiPlatformCreativeGenerator Analysis',
