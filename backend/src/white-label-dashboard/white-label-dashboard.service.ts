...
export class WhiteLabelDashboardService {
  constructor(
    @InjectRepository(WhiteLabelDashboard)
    private readonly repository: Repository<WhiteLabelDashboard>,
  ) {}

  async create(userId: string, data: Partial<WhiteLabelDashboard>): Promise<WhiteLabelDashboard> {
  ...
  async findAllByUser(userId: string): Promise<WhiteLabelDashboard[]> {
  ...
  async findOne(id: string): Promise<WhiteLabelDashboard> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`WhiteLabelDashboard with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<WhiteLabelDashboard>): Promise<WhiteLabelDashboard> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'WhiteLabelDashboard Analysis',
