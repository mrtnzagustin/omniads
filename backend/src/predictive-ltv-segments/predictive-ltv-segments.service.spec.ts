...
describe('PredictiveLtvSegmentsService', () => {
  let service: PredictiveLtvSegmentsService;
  let repository: Repository<PredictiveLtvSegments>;
  ...
  const mockEntity: Partial<PredictiveLtvSegments> = {
    id: '123',
    userId: 'user1',
    name: 'Test Entity',
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        PredictiveLtvSegmentsService,
        {
          provide: getRepositoryToken(PredictiveLtvSegments),
          useValue: mockRepository,
        },
      ],
    }).compile();

    service = module.get<PredictiveLtvSegmentsService>(PredictiveLtvSegmentsService);
    repository = module.get<Repository<PredictiveLtvSegments>>(getRepositoryToken(PredictiveLtvSegments));
  });
