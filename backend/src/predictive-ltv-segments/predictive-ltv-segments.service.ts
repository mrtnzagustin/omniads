...
export class PredictiveLtvSegmentsService {
  constructor(
    @InjectRepository(PredictiveLtvSegments)
    private readonly repository: Repository<PredictiveLtvSegments>,
  ) {}

  async create(userId: string, data: Partial<PredictiveLtvSegments>): Promise<PredictiveLtvSegments> {
  ...
  async findAllByUser(userId: string): Promise<PredictiveLtvSegments[]> {
  ...
  async findOne(id: string): Promise<PredictiveLtvSegments> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`PredictiveLtvSegments with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<PredictiveLtvSegments>): Promise<PredictiveLtvSegments> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'PredictiveLtvSegments Analysis',
