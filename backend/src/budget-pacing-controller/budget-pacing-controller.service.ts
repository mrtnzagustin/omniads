...
export class BudgetPacingControllerService {
  constructor(
    @InjectRepository(BudgetPacingController)
    private readonly repository: Repository<BudgetPacingController>,
  ) {}

  async create(userId: string, data: Partial<BudgetPacingController>): Promise<BudgetPacingController> {
  ...
  async findAllByUser(userId: string): Promise<BudgetPacingController[]> {
  ...
  async findOne(id: string): Promise<BudgetPacingController> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) {
      throw new NotFoundException(`BudgetPacingController with ID ${id} not found`);
    }
    return entity;
  }

  async update(id: string, data: Partial<BudgetPacingController>): Promise<BudgetPacingController> {
  ...
  async analyze(id: string): Promise<any> {
    ...
    const analysis = {
      summary: 'BudgetPacingController Analysis',
