...
As a marketing manager, I can use Contextual Targeting Engine to improve campaign performance with Post-cookie targeting using NLP and page context, so that I achieve better targeting and engagement without relying on third-party data.

**Why this priority**: This addresses key 2025 marketing challenges around privacy and performance.

**Independent Test**: Configure Contextual Targeting Engine on test campaign, verify it delivers expected functionality and improves key metrics by 20%+.
...
- **FR-001**: System MUST provide core Contextual Targeting Engine functionality as described.
...
- **031contextualtargetingengineConfig**: Feature configuration per workspace/campaign
- **031contextualtargetingengineActivity**: Activity and decision logs
- **031contextualtargetingengineMetrics**: Performance metrics and outcomes
...
```sql
CREATE TABLE 031_contextual_targeting_engine_configs (
...
CREATE TABLE 031_contextual_targeting_engine_activities (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  config_id UUID REFERENCES 031_contextual_targeting_engine_configs(id),
  workspace_id VARCHAR(255) NOT NULL,
...
CREATE TABLE 031_contextual_targeting_engine_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  config_id UUID REFERENCES 031_contextual_targeting_engine_configs(id),
  workspace_id VARCHAR(255) NOT NULL,
  date DATE NOT NULL,
...
```
POST   /api/v1/031-contextual-targeting-engine                   - Create/activate feature
GET    /api/v1/031-contextual-targeting-engine/:id                - Get configuration
PATCH  /api/v1/031-contextual-targeting-engine/:id                - Update configuration
DELETE /api/v1/031-contextual-targeting-engine/:id                - Deactivate feature
GET    /api/v1/031-contextual-targeting-engine/:id/metrics        - Get performance metrics
GET    /api/v1/031-contextual-targeting-engine/:id/activities     - Get activity history
```
