# Feature Specification: Cross-Platform Frequency Management
...
As a marketing manager, I can use Cross-Platform Frequency Management to improve campaign performance with Unified frequency capping across all platforms, so that I achieve better targeting and engagement without relying on third-party data.

**Why this priority**: This addresses key 2025 marketing challenges around privacy and performance.

**Independent Test**: Configure Cross-Platform Frequency Management on test campaign, verify it delivers expected functionality and improves key metrics by 20%+.
...
- **FR-001**: System MUST provide core Cross-Platform Frequency Management functionality as described.
...
- **034crossplatformfrequencymgmtConfig**: Feature configuration per workspace/campaign
- **034crossplatformfrequencymgmtActivity**: Activity and decision logs
- **034crossplatformfrequencymgmtMetrics**: Performance metrics and outcomes
...
```sql
CREATE TABLE 034_cross_platform_frequency_mgmt_configs (
...
CREATE TABLE 034_cross_platform_frequency_mgmt_activities (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  config_id UUID REFERENCES 034_cross_platform_frequency_mgmt_configs(id),
  workspace_id VARCHAR(255) NOT NULL,
...
CREATE TABLE 034_cross_platform_frequency_mgmt_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  config_id UUID REFERENCES 034_cross_platform_frequency_mgmt_configs(id),
  workspace_id VARCHAR(255) NOT NULL,
  date DATE NOT NULL,
...
```
POST   /api/v1/034-cross-platform-frequency-mgmt                   - Create/activate feature
GET    /api/v1/034-cross-platform-frequency-mgmt/:id                - Get configuration
PATCH  /api/v1/034-cross-platform-frequency-mgmt/:id                - Update configuration
DELETE /api/v1/034-cross-platform-frequency-mgmt/:id                - Deactivate feature
GET    /api/v1/034-cross-platform-frequency-mgmt/:id/metrics        - Get performance metrics
GET    /api/v1/034-cross-platform-frequency-mgmt/:id/activities     - Get activity history
```
